...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const svg = await satori(normalizeFlex(toVNode(v2.html)) as any, { width, height, fonts: fonts as any });
    let png: Buffer = Buffer.from(new Resvg(svg, { fitTo: { mode: 'width', value: width } }).render().asPng());
    if (photoRgba) { try { png = compositeOverPhotoV2(png, photoRgba, width, height); backgroundStatus = 'composited'; } catch { backgroundStatus = 'skip_composite_failed'; } }
